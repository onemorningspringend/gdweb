...
window["invoker"].FSSC_edit = function () {
    return function (fsmode) {
        ...
        return gdself.context.invoke([{
            target: 'GDWebChangeCardController',
            methodName: 'ChangeEdit',
            params: []
        }
        ]).then(function () {
            gdself.context.view().transitInvoke('Edit', [{
                target: '',
                methodName: '',
                params: []
            }]);
...
window["invoker"].FSSC_save = function () {
    return function () {
        ...
        return gdself.context.invoke([{
            target: 'GDWebChangeCardController',
            methodName: 'ChangeSaveforFSSC',
            params: []
        }
        ]).then(function (data) {
            var params = [];
            params[0] = true;
            var cardInstanceinner = gdself.context.injector.get('$model')('DM_GDWEB_GDBGJL_CardInstance');
            var controllerinner = gdself.context.controllers["GDWebChangeCardController"];
...
window["invoker"].FSSC_saveafter = function () {
    return function () {
        ...
        return selfinner.context.invoke([{
            target: 'GDWebChangeCardController',
            methodName: 'SaveAfter',
            params: []
        }]).then(function () {
            selfinner.context.view().transitInvoke('Cancel', [{
                target: '',
                methodName: '',
                params: []
            }]);
...
function GDWEBCHANGEASSET_ListSubmitBefore(djnm, djlx, contextFSSC) {
    var def = $.Deferred();
    def.resolve(true);
    return def.promise();
};
